          ...
          <AnimatePresence mode="wait">
            ...
            {!initialChecking && verifiedInvoice && (
              ...
              >
                ...
                <div className="w-full border-b border-slate-200/90 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md sticky top-16 z-30 shadow-xs">
                  {/* Row 1: Document Identification & Primary Actions (Mobile, Tablet & Desktop) */}
                  <div className="px-3 sm:px-6 lg:px-8 py-2.5 sm:py-3 flex items-center justify-between gap-2 sm:gap-4">
                    {/* Left: Search / Back, Verified Badge & Clickable Invoice ID */}
                    <div className="flex items-center gap-1.5 sm:gap-3 min-w-0">
                      <button
                        type="button"
                        onClick={handleReset}
                        className="px-2.5 py-1.5 rounded-xl text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 transition-all flex items-center gap-1.5 text-xs font-semibold cursor-pointer shrink-0 border border-slate-200/70 dark:border-slate-700/80 active:scale-95 shadow-xs"
                        title="Search or verify another invoice"
                      >
                      ...
                      <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                        <span className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 text-xs font-semibold shrink-0">
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400 shrink-0" />
                          <span className="hidden md:inline">Verified A4</span>
                        ...
                        <button
                          ...
                          className="font-mono text-xs sm:text-sm font-bold text-slate-900 dark:text-white bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 px-2 py-0.5 rounded-md transition-colors truncate max-w-[130px] sm:max-w-[200px] border border-slate-200/60 dark:border-slate-700/60 cursor-pointer"
                    ...
                    {/* Right on Desktop: Unified Zoom + Actions; On Mobile/Tablet: Actions Only */}
                    <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                      {/* Desktop Only: Zoom Controller integrated in Row 1 */}
                      <div className="hidden lg:flex items-center bg-slate-100/90 dark:bg-slate-800/90 p-0.5 rounded-xl border border-slate-200/80 dark:border-slate-700/80 text-xs shadow-xs mr-1">
                        <button
                          type="button"
                          onClick={() => setZoom(z => Math.max(0.35, Number((z - 0.1).toFixed(2))))}
                          className="p-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 cursor-pointer transition-all active:scale-95"
                          title="Zoom Out"
                        >
                          <ZoomOut className="w-3.5 h-3.5" />
                        </button>
                        <span className="px-2 font-mono text-[11px] text-slate-800 dark:text-slate-200 min-w-[42px] text-center font-bold">
                          {Math.round(zoom * 100)}%
                        </span>
                        <button
                          type="button"
                        ...
                        <button
                          ...
                          className="px-2 py-1 rounded-lg hover:bg-white dark:hover:bg-slate-700 text-indigo-600 dark:text-indigo-400 font-semibold cursor-pointer text-[10px] border-l border-slate-200 dark:border-slate-700 transition-all active:scale-95"
                        ...
                        <button
                          type="button"
                          onClick={() => setZoom(1.0)}
                          className="p-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 cursor-pointer transition-all active:scale-95"
                      ...
                      <button
                        ...
                        className="p-2 sm:p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all cursor-pointer shadow-xs active:scale-95"
                        title="Copy Public Link to Invoice"
                      >
                      ...
                      <button
                        ...
                        className="p-2 sm:p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-all cursor-pointer shadow-xs active:scale-95"
                      ...
                      <button
                        type="button"
                        onClick={handlePrint}
                        className="px-3 sm:px-5 py-2 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 text-white font-bold rounded-xl shadow-md shadow-indigo-600/20 text-xs sm:text-sm flex items-center gap-1.5 sm:gap-2 transition-all active:scale-95 cursor-pointer shrink-0"
                      >
                  ...
                  {/* Row 2 on Mobile & Tablet: Clean Document Precision Zoom Bar */}
                  <div className="lg:hidden px-3 sm:px-6 py-2 bg-slate-50/90 dark:bg-slate-950/80 border-t border-slate-200/70 dark:border-slate-800/70 flex items-center justify-between gap-2">
                    <span className="text-[11px] font-medium text-slate-500 dark:text-slate-400 flex items-center gap-1.5 shrink-0">
                      <ScanLine className="w-3.5 h-3.5 text-indigo-500" />
                      <span>Standard A4 View</span>
                    </span>

                    {/* Mobile/Tablet Centered Zoom Controls Bar */}
                    <div className="flex items-center bg-white dark:bg-slate-800 p-0.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs shadow-xs">
                      <button
                        type="button"
                        onClick={() => setZoom(z => Math.max(0.35, Number((z - 0.1).toFixed(2))))}
                        className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 cursor-pointer transition-all active:scale-95"
                        title="Zoom Out"
                      >
                        <ZoomOut className="w-3.5 h-3.5" />
                      </button>
                      <span className="px-2 font-mono text-[11px] text-slate-700 dark:text-slate-300 min-w-[38px] text-center font-bold">
                        {Math.round(zoom * 100)}%
                      </span>
                      <button
                        type="button"
                      ...
                      <button
                        type="button"
                        onClick={handleAutoFit}
                        className="px-2.5 py-1 rounded-lg bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/50 dark:hover:bg-indigo-900/60 text-indigo-600 dark:text-indigo-400 font-semibold cursor-pointer text-[10px] border-l border-slate-200 dark:border-slate-700 transition-all active:scale-95"
                      ...
                      <button
                        type="button"
                        onClick={() => setZoom(1.0)}
                        className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-500 cursor-pointer transition-all active:scale-95"
                        title="Reset 100%"
                      >
                        <RotateCcw className="w-3.5 h-3.5" />
                      </button>
                ...
                {/* Document Viewport - Clean, Centered Full-Width Paper Canvas */}
                <div
                  ref={containerRef}
                  className="flex-1 w-full bg-slate-100/90 dark:bg-slate-950 py-6 sm:py-8 lg:py-12 px-2 sm:px-4 lg:px-8 flex justify-center items-start overflow-x-auto min-h-[calc(100vh-140px)]"
                >
                  <div className="flex flex-col items-center justify-start mx-auto transition-all duration-200">
                    {/* A4 Document Paper Frame with Crisp Margins & Multi-layer Shadows */}
                    <div
                      style={{
                      ...
                      className="transition-all duration-150 shadow-[0_10px_35px_-5px_rgba(0,0,0,0.1),0_20px_45px_-10px_rgba(0,0,0,0.12)] dark:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.8)] rounded-sm bg-white border border-slate-300/80 dark:border-slate-800 ring-1 ring-black/5"
                    >
                      <div
                        style={{
